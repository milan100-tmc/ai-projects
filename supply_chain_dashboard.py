    ...
    with st.spinner("Analysing supply chain data..."):
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"""Analyse this supplier performance data and give me the 5 most critical insights a supply chain director needs to act on immediately:
...
Focus on: risk suppliers, cost impact, recommended actions, and which suppliers to prioritise or replace."""}
            ]
...
if prompt := st.chat_input("e.g. Which supplier is highest risk? What's causing stockouts?"):
    ...
    with st.chat_message("assistant"):
        response = client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[
                {"role": "system", "content": f"You are a senior supply chain analyst. Answer with specific numbers and actionable recommendations:\n{data_context}"},
